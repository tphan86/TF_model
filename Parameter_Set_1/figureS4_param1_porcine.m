%% Resolution Matrix for ktr vs pCa in porcine LV - parameter set 1
...
Parms_1 = 10; % the number of fitting parameters
...
[fitted_para_1,resnorm_1,residual_1,exitflag_1,output_1,lambda_1,jacobian_1] = lsqcurvefit( ...
    @rate_force_redev_1,para_fit_1,pCa_porcine_LV,ktr_mean_porcine_LV,lower_bound_1,...
    upper_bound_1,opts);
...
% saveas(gcf, 'data_res_matrix_ktr_porcine_1.jpg');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute the model resolution matrix
% model_resolution_matrix_1 = pinv(jacobian_1' * jacobian_1) * (jacobian_1' * jacobian_1);
lambda = 1e-6;  % Adjust this value as needed
J = jacobian_1;
model_resolution_matrix_1 = pinv(J'*J) * (J'*J);

% Plot the model resolution matrix as a heatmap
figure;
model_res_matrix_1_resized = imresize(model_resolution_matrix_1, [10, 10]);
...
% % Save the heatmap as a JPEG file
% saveas(gcf, 'model_res_matrix_ktr_porcine_1.jpg');